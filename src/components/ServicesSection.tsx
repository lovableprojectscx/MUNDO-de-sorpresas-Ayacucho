...
import { MessageCircle, Wand2, CalendarDays } from "lucide-react";
                  ...
                  <div>
                    {service.type === "reservation" ? (
                      <button
                        ...
                        className="inline-flex w-full sm:w-auto items-center justify-center gap-2 px-6 py-3 rounded-full bg-gradient-to-r from-[hsl(280,60%,45%)] to-[hsl(330,70%,55%)] text-white font-body font-bold text-sm shadow-lg hover:shadow-xl hover:opacity-95 transition-all duration-300 hover:-translate-y-0.5 group/btn"
                      >
                        {service.cta}
                        <service.icon className="w-4 h-4 group-hover/btn:scale-110 transition-transform" />
                      </button>
                    ) : (
                      <Link
                        to={service.link!}
